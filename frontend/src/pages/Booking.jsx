...
export default function Booking() {
  ...
  const tripDays = dayCount(startDate, endDate);
  ...
  const shareUrl = bookingId && bookingId !== "CONFIRMED"
    ? `${window.location.origin}/shared-trip/${bookingId}`
    : null;
  ...
  const downloadPDF = () =>
    downloadItineraryPDF({ destination, budget, startDate, endDate, tripData, bookingId });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (code === 401 || raw.toLowerCase().includes("token") || raw.toLowerCase().includes("authoriz")) {
        msg = "You need to sign in to save a trip. Please log in or create an account.";
      } else if (code === 403) {
        msg = "You don't have permission to save this trip.";
      } else {
    ...
    <>
      ...
      <div className="bk-root">
        ...
        <div className="bk-empty">
          ...
          <p className="bk-empty-sub">Use our AI Planner to generate your itinerary, then save it here.</p>
    ...
    <>
      ...
      <div className="bk-root">
        ...
        <div className="bk-success">
          ...
          <div className="bk-success-title">Trip Saved!</div>
          <p className="bk-success-sub">Your trip to <strong style={{textTransform:"capitalize"}}>{destination}</strong> is saved to your dashboard.</p>

          <div style={{margin:"16px 0", display:"flex", flexWrap:"wrap", gap:"8px", justifyContent:"center"}}>
            {startDate && <span className="bk-pill" style={{background:"rgba(245,158,11,0.12)", border:"1px solid rgba(245,158,11,0.25)", borderRadius:"100px", padding:"5px 14px", fontSize:"12px", fontWeight:700, color:"#b45309"}}>📅 {startDate} → {endDate}</span>}
            {budget && <span className="bk-pill" style={{background:"rgba(16,185,129,0.12)", border:"1px solid rgba(16,185,129,0.25)", borderRadius:"100px", padding:"5px 14px", fontSize:"12px", fontWeight:700, color:"#047857"}}>💰 ₹{Number(budget).toLocaleString("en-IN")}</span>}
            {dayCount > 0 && <span className="bk-pill" style={{background:"rgba(99,102,241,0.12)", border:"1px solid rgba(99,102,241,0.25)", borderRadius:"100px", padding:"5px 14px", fontSize:"12px", fontWeight:700, color:"#4338ca"}}>🌙 {dayCount} Days</span>}
          </div>

          {bookingId && <div className="bk-booking-id">Booking ID: {bookingId}</div>}

          {shareUrl && (
            <div style={{margin:"14px 0", display:"flex", gap:"8px", justifyContent:"center"}}>
              <input readOnly value={shareUrl} style={{padding:"8px 12px", borderRadius:"10px", border:"1px solid rgba(0,0,0,0.1)", fontSize:"12px", fontFamily:"DM Sans,sans-serif", background:"rgba(255,255,255,0.85)", color:"#374151", maxWidth:"260px", textOverflow:"ellipsis", overflow:"hidden", whiteSpace:"nowrap"}} />
              <button onClick={copyLink} style={{padding:"8px 14px", borderRadius:"10px", border:"none", background: copied ? "rgba(16,185,129,0.15)" : "rgba(245,158,11,0.12)", color: copied ? "#047857" : "#b45309", fontWeight:700, fontSize:"12px", cursor:"pointer", fontFamily:"DM Sans,sans-serif", whiteSpace:"nowrap"}}>{copied ? "Copied!" : "Copy Link"}</button>
            </div>
          )}

          <div className="bk-success-actions">
            <button className="bk-btn-dash" onClick={() => navigate("/dashboard")}>📋 My Trips</button>
            <button className="bk-btn-home" onClick={downloadPDF}>📄 Download PDF</button>
      ...
      <div className="bk-root">
        ...
        <div className="bk-card">
          <div className="bk-hero">
            ...
            <div className="bk-hero-label">Review & Save</div>
            ...
            <div className="bk-hero-pills">
              ...
              {tripDays > 0 && <span className="bk-pill">🌙 {tripDays} Days</span>}
          ...
          <div className="bk-body">
            <div>
              ...
              <div className="bk-summary">
                <div className="bk-stat"><div className="bk-stat-label">Duration</div><div className="bk-stat-val amber">{tripDays || "—"} Days</div></div>
            ...
            <button className="bk-cta" onClick={handleSubmit} disabled={status==="loading"}>
              {status === "loading" ? "⏳ Saving…" : "✈️ Save This Trip"}
            </button>
          </div>
        </div>
      </div>
