...
export default function Booking() {
  ...
  const downloadPDF = async () => {
    ...
    doc.setFontSize(22);
    doc.text(`Trip to ${destination}`, 16, 30);
    doc.setFontSize(12);
    let y = 42;
    if (startDate) { doc.text(`📅 ${startDate} → ${endDate || "—"}`, 16, y); y += 8; }
    if (budget) { doc.text(`💰 Budget: ₹${Number(budget).toLocaleString("en-IN")}`, 16, y); y += 8; }
    doc.text(`👤 ${form.fullName} · ${form.email}`, 16, y); y += 12;
    doc.setFontSize(10); doc.setTextColor(100);
    doc.text("Generated by TripNow", 16, 260);
    doc.text(`Booking: ${bookingId}`, 16, 268);
    doc.save(`${destination}-tripnow-trip.pdf`);
  };
