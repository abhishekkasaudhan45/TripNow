...
// FIXES:
//   1. "View Dashboard" button: changed from text-white (invisible on light bg) to dark text with proper border
//   2. Button cluster wraps properly on all screen sizes
...
export default function PlanTrip() {
  ...
  const downloadPDF = () => {
    ...
    const doc = new jsPDF();
    let y = 10;
    doc.setFontSize(16); doc.text(`Trip to ${destination}`, 10, y); y += 10;
    doc.setFontSize(12); doc.text(`Dates: ${startDate} → ${endDate}`, 10, y); y += 10;
    doc.text(`Budget: ₹${budget}`, 10, y); y += 10;
    tripData.days?.forEach(day => {
      doc.setFontSize(13); doc.text(`Day ${day.day}: ${day.title}`, 10, y); y += 6;
      doc.setFontSize(10);
      const m = doc.splitTextToSize(`Morning: ${day.morning}`, 180); doc.text(m, 10, y); y += m.length * 5;
      const a = doc.splitTextToSize(`Afternoon: ${day.afternoon}`, 180); doc.text(a, 10, y); y += a.length * 5;
      const e = doc.splitTextToSize(`Evening: ${day.evening}`, 180); doc.text(e, 10, y); y += e.length * 5 + 3;
  ...
  return (
    <>
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .plan-trip-container {
          --font-head:'Instrument Serif',Georgia,serif; --font-ui:'Space Grotesk',sans-serif;
          --bg:transparent; --surface:rgba(255,255,255,0.65); --surface-2:rgba(255,255,255,0.4); --surface-3:rgba(255,255,255,0.8);
        ...
        .shell { background:rgba(255,255,255,0.55); backdrop-filter:blur(24px); -webkit-backdrop-filter:blur(24px); width:100%; max-width:1200px; border-radius:16px; overflow:hidden; border:1px solid rgba(255,255,255,0.6); box-shadow:0 20px 50px rgba(0,0,0,0.05),inset 0 0 0 1px rgba(255,255,255,0.5); display:flex; flex-direction:column; }
        ...
        /* ✅ FIX: was text-white on light bg — now slate text with visible border */
        .btn-dashboard {
          padding:8px 16px; border-radius:10px; font-size:13px; font-weight:700; cursor:pointer;
        ...
        .btn-book {
          padding:8px 20px; border-radius:10px; font-size:13px; font-weight:800; cursor:pointer;
        ...
        .day-header { display:flex; align-items:center; cursor:pointer; padding:0; }
        .day-num { width:56px; height:56px; display:flex; align-items:center; justify-content:center; font-weight:700; font-size:15px; color:var(--muted); border-right:1px solid rgba(0,0,0,0.05); flex-shrink:0; background:rgba(255,255,255,0.5); }
        ...
        .tip-card { background:rgba(255,255,255,0.7); border:1px solid rgba(0,0,0,0.05); border-radius:8px; padding:14px 16px; display:flex; gap:12px; transition:transform 0.2s; }
        .tip-card:hover { transform:translateY(-2px); }
        .tip-num { font-size:13px; font-weight:800; color:var(--muted); flex-shrink:0; min-width:20px; }
          ...
          <div className="topbar">
            ...
            <div className="topbar-right">
              {loading
                ...
                : <><div className="tag tag-emerald"><div className="dot" /> AI Ready</div><div className="tag tag-amber">{dayCount} Days</div></>
              }
            </div>
          </div>
          ...
          <div className="layout">
            ...
            <div className="main">
              <div className="page-header">
                ...
                {/* ✅ FIXED BUTTON CLUSTER — all buttons now clearly visible */}
                {!loading && !error && tripData && (
                  <div className="btn-cluster">
                    <button className="btn-pdf" onClick={downloadPDF}>
                      📄 Download PDF
                    </button>
                    {/* ✅ FIX: was invisible white text on light bg — now slate/dark text */}
              ...
              {!loading && !showSuccess && error && (
                <div className="content" style={{ alignItems:'center', justifyContent:'center' }}>
                  <div style={{ textAlign:'center', background:'var(--rose-dim)', padding:'24px', borderRadius:'12px', border:'1px solid rgba(244,63,94,0.3)' }}>
                    ...
                    <button onClick={() => navigate(-1)} style={{ marginTop:'20px', background:'var(--rose)', color:'#fff', border:'none', padding:'10px 20px', borderRadius:'6px', cursor:'pointer', fontWeight:'600' }}>Go Back</button>
                  </div>
                </div>
              )}
                ...
                <>
                  {tripData.bestTimeToVisit && activeTab === "Itinerary" && (
                    <div className="alert-bar">
                      ...
                      <div className="alert-text">Best time to visit: <b style={{ color:"var(--text)" }}>{tripData.bestTimeToVisit}</b></div>
                    </div>
                  )}
                          ...
                          {tripData.days?.map((day, i) => (
                            <div key={i} className={`day-row ${openDays[i] ? "open" : ""}`}>
                              <div className="day-header" onClick={() => toggleDay(i)}>
                                ...
                                <div className="day-title-wrap"><div className="day-title">{day.title || `Exploring ${destination}`}</div></div>
                                <div className={`day-chevron ${openDays[i] ? "open" : ""}`}>▾</div>
                              </div>
                              <div className={`day-body ${openDays[i] ? "open" : ""}`}>
                                <div className="day-slot">
                                  <div className="slot-label" style={{color:'var(--amber-text)'}}><div className="slot-dot" style={{background:'var(--amber)'}}/>Morning</div>
                                  <div className="slot-text">{day.morning}</div>
                                </div>
                                <div className="day-slot">
                                  <div className="slot-label" style={{color:'var(--sky-text)'}}><div className="slot-dot" style={{background:'var(--sky)'}}/>Afternoon</div>
                                  <div className="slot-text">{day.afternoon}</div>
                                </div>
                                <div className="day-slot">
                                  <div className="slot-label" style={{color:'var(--violet)'}}><div className="slot-dot" style={{background:'var(--violet)'}}/>Evening</div>
                                  ...
                                  {day.food?.length > 0 && (
                                    <div style={{marginTop:'12px',padding:'10px',background:'rgba(245,158,11,0.1)',borderRadius:'6px',border:'1px solid rgba(245,158,11,0.2)'}}>
                                      ...
                                      {day.food.map((f, fi) => <div key={fi} style={{fontSize:'12px',color:'var(--text)',marginTop:'4px',fontWeight:'500'}}>• {f}</div>)}
                                    </div>
                                  )}
                                </div>
                              </div>
